        ...
        <ul class="todo" id="todo">
            @foreach ($tasks as $task)
                <li style="display: flex; align-items: center;">
                    <!-- Checkbox -->
                    <form method="POST" action="{{ route('tasks.toggle', $task->id) }}" style="margin-right: 10px;">
                        @csrf
                        <input type="checkbox" onchange="this.form.submit()" {{ $task->is_done ? 'checked' : '' }}>
                    </form>

                    <!-- Task Name -->
                    <span style="{{ $task->is_done ? 'text-decoration: line-through; color: gray;' : '' }} flex: 1;">
                        {{ $task->name }}
                    </span>

                    <!-- Created Date & Delete Button -->
                    <form action="{{ route('item.destroy', $task->id) }}" method="post" style="display: inline; margin-left: auto;">
                        @csrf
                        @method('DELETE')
                        <span style="color: gray; margin-left: 10px;">Created at: {{ $task->created_at }}</span>
                        <button class="delete">
